...
async fn retrieve_file(
    ...
    Extension(ass): Extension<ass::Ass>,
    Query(query): Query<FileQuery>,
) -> Response {
    let api_key_hash = format!("{:x}", md5::compute(&api_key));
    let stored = match ass.get(&query.file_id).await {
        Ok(stored) => stored,
        Err(_) => {
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
                "ASS is having a moment.",
            )
                .into_response();
        }
    };
    ...
    if let Some(file) = stored {
        if file.is_public || file.owner_key_hash == api_key_hash {
...
async fn upload_file(
    ...
    for (filename, file_data) in files_to_process {
        ...
        let size_bytes = match ass
            .put(&file_id, &filename, file_data, is_public, &api_key_hash)
            .await
        {
            Ok(size) => size,
            Err(error) => {
                return (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    format!("Failed to store file in ASS -- {error}"),
                )
                    .into_response();
            }
        };
        ...
        file_infos.push(FileInfo {
            ...
            file_url: ass::public_file_url(&file_id),
...
async fn get_public_file(
    ...
    let stored = match ass.get(&file_id).await {
        Ok(stored) => stored,
        Err(_) => {
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
    ...
    if let Some(file) = stored.filter(|file| file.is_public) {
        return Response::builder()
            .status(StatusCode::OK)
            .header("Content-Type", file.content_type)
            .header("X-Powered-By", "AvgDB ASS on a Railway bucket")
